...
To get a simple N-0/N-1 computation, we can use [`run_solver`][toop_engine_dc_solver.jax.run_solver]:
...
You can use [`convert_topo_sel_sorted`][toop_engine_dc_solver.jax.convert_topo_sel_sorted] and [`convert_branch_topo_vect`][toop_engine_dc_solver.jax.convert_branch_topo_vect] to convert between the two formats of storing a branch topology.

Injection topologies are represented via the action set in JAX. For custom injection combinations you can use [`convert_inj_topo_vect`][toop_engine_dc_solver.jax.convert_inj_topo_vect] together with [`convert_inj_candidates`][toop_engine_dc_solver.jax.convert_inj_candidates] to build inputs for the solver.
...
The possible aggregation metrics can be found [`here`][toop_engine_interfaces.types.MetricType]. Here is a brief summary:
...
| "pst_startup_cost"                        | Number of PST tap indices that changed compared to the initial taps (implicit unit cost, weighted through `target_metrics`)                                                |
...
The absolute values of the N0-N1 Difference are calculated as part of the initial load flow calculation ([`run_initial_loadflow`][toop_engine_dc_solver.preprocess.convert_to_jax.run_initial_loadflow]) during the preprocess function [`load_grid`][toop_engine_dc_solver.preprocess.convert_to_jax.load_grid] and stored inside the static information as part of the branch_limits with the key `'n0_n1_max_diff'`.

To use the N0-N1 Difference in the optimizer, the metric `"n0_n1_delta"` has to be included in the target_metrics.

### Postprocessing

The repository offers some postprocessing routines as well, they are bundled in the [`postprocess`][toop_engine_dc_solver.postprocess] module. Depending on the backend you use, you will either need a [`PandapowerRunner`][toop_engine_dc_solver.postprocess.PandapowerRunner] or a [`PowsyblRunner`][toop_engine_dc_solver.postprocess.PowsyblRunner]. These classes abstract away the loadflow computation feature of the grid modelling packages.

Furthermore, methods from the [`postprocess`][toop_engine_dc_solver.postprocess] module can be used to extract the relevant information from the network data and apply the topologies to the grid. The following code snippet shows how to use the postprocessing module:

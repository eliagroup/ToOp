...
1. **[`NetworkGraphData`][toop_engine_importer.network_graph.NetworkGraphData]**
   ...
   - **[`NodeSchema`][toop_engine_importer.network_graph.NodeSchema]**: Defines all the nodes.
        [`NODE_TYPES`][toop_engine_importer.network_graph.NODE_TYPES]: "busbar" a true physical busbar you can touch (but probably shouldn't)
                   "node" is a connection point between elements or fictive nodes where two TSO meet (border nodes)
   - **[`SwitchSchema`][toop_engine_importer.network_graph.SwitchSchema]**: This is core information. If no switches are present, the mapping will do nothing.
        [`SWITCH_TYPES`][toop_engine_importer.network_graph.SWITCH_TYPES]: "DISCONNECTOR" (Power Switch), "BREAKER" (Non Power Switch)
   - **[`BranchSchema`][toop_engine_importer.network_graph.BranchSchema]**: Optional. If only a substation is represented, a branch would be represented in the [`NodeAssetSchema`][toop_engine_importer.network_graph.NodeAssetSchema] at the outer edge of the network.
        [`BRANCH_TYPES`][toop_engine_importer.network_graph.BRANCH_TYPES] depend on the framework you use.
        [`BRANCH_TYPES_POWSYBL`][toop_engine_importer.network_graph.BRANCH_TYPES_POWSYBL]: "LINE", "TWO_WINDING_TRANSFORMER", "PHASE_SHIFTER", "TWO_WINDING_TRANSFORMER_WITH_TAP_CHANGER", "THREE_WINDINGS_TRANSFORMER",
        [`BRANCH_TYPES_PANDAPOWER`][toop_engine_importer.network_graph.BRANCH_TYPES_PANDAPOWER]: "line", "trafo", "trafo3w", "dcline", "tcsc", "impedance"
   - **[`NodeAssetSchema`][toop_engine_importer.network_graph.NodeAssetSchema]**: Optional. Mainly used to represent node assets like generators, loads, etc. It can also be used to represent border lines of the network (either of a substation or a line leaving the network).
   - **[`HelperBranchSchema`][toop_engine_importer.network_graph.HelperBranchSchema]**: A concept mainly introduced to support the exact layout of `powsybl.net.get_node_breaker_topology(substation_info.voltage_level_id)`. For example, a `BusbarSection` in the powsybl NodeBreakerTopology is a dead-end node. The true connection hub is a helper node, where all switches are connected (again with a helper node in between).

2. **[`BusbarConnectionInfo`][toop_engine_importer.network_graph.BusbarConnectionInfo] and **[`EdgeConnectionInfo`][toop_engine_importer.network_graph.EdgeConnectionInfo]**
   These data classes are initialized blank. The data gets filled step by step using the [`run_default_filter_strategy`][toop_engine_importer.network_graph.run_default_filter_strategy]. Most of the data is used for the [AssetTopology][toop_engine_interfaces.asset_topology], and some for tracking dependencies between the filter process.
...
1. **Extract the Grid or a Station into the NetworkGraphData Format:**
   ...
   - [`powsybl_station_to_graph`][toop_engine_importer.network_graph.get_node_breaker_topology_graph]

2. **Enrich NetworkGraphData with Generic Data:**
   - Add [`filter_weights`][toop_engine_importer.network_graph.set_all_weights] to NetworkGraphData – preparation for the filter process.
     Sets weights based on the three DataFrames:
     - [`NetworkGraphData.branches`][toop_engine_importer.network_graph.NetworkGraphData]
     - [`NetworkGraphData.switches`][toop_engine_importer.network_graph.NetworkGraphData]
     - [`NetworkGraphData.helper_branches`][toop_engine_importer.network_graph.NetworkGraphData]

3. **Generate the Graph:**
   A `networkx.Graph` is generated based on the [`NetworkGraphData`][toop_engine_importer.network_graph.NetworkGraphData] class. [`BusbarConnectionInfo`][toop_engine_importer.network_graph.BusbarConnectionInfo] (node only) and [`EdgeConnectionInfo`][toop_engine_importer.network_graph.EdgeConnectionInfo] (edge only) are initialized for all elements.
   - Add a Substation ID if missing (pandapower has no substation logic, but CGMES import does).

4. **Run [`run_default_filter_strategy`][toop_engine_importer.network_graph.run_default_filter_strategy]:**
   Here the mapping logic is implemented. The order of these steps is important, as there are dependencies between the steps.
   - **[`set_switch_busbar_connection_info`][toop_engine_importer.network_graph.set_switch_busbar_connection_info]**
   ...
   - **[`set_bay_weights`][toop_engine_importer.network_graph.set_bay_weights]**
   ...
   - **[`set_empty_bay_weights`][toop_engine_importer.network_graph.filter_strategy.empty_bay.set_empty_bay_weights]**
   ...
   - **[`set_connectable_busbars`][toop_engine_importer.network_graph.set_connectable_busbars]**
     ...
     Sets the connectable busbars result from `calculate_connectable_busbars` to the [`BusbarConnectionInfo`][toop_engine_importer.network_graph.BusbarConnectionInfo]. It sets `connectable_busbars` and `connectable_busbars_node_ids` in the same order.
   - **[`set_all_busbar_coupling_switches`][toop_engine_importer.network_graph.filter_strategy.switches.set_all_busbar_coupling_switches]**
   ...
   - **[`set_zero_impedance_connected`][toop_engine_importer.network_graph.set_zero_impedance_connected]**

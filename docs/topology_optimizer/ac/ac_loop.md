...
Furthermore a logger is bound to the epoch number, so all logger calls inside the loop will have the epoch attached in the structlog.


### Poll the Results Topic

The loop begins by calling `poll_results_topic`.

This import step has two purposes: it moves newly produced DC topologies into the AC worker's local database, and keeps the worker's repertoire view synchronized with the upstream DC process. It happens once per epoch before any new AC topology is pulled. The reason for doing this eagerly is that the AC loop is not an isolated optimizer with a fixed search space; it is a downstream consumer of a continuously evolving DC repertoire, so delaying the import would directly reduce the usefulness of the AC selection stage.
...
### Decide When to Flush Survivors

...
- **`worker/`**: Kafka-based worker infrastructure for distributed optimization
  - [`optimizer.py`][toop_engine_topology_optimizer.dc.worker.optimizer]: Core optimization logic and genetic algorithm execution
  - [`worker.py`][toop_engine_topology_optimizer.dc.worker.worker]: Kafka consumer/producer for distributed coordination
...
- **Parameters**: Configuration classes for [`DC`][toop_engine_topology_optimizer.interfaces.messages.dc_params.DCOptimizerParameters] and [`AC`][toop_engine_topology_optimizer.interfaces.messages.ac_params.ACOptimizerParameters] optimizer parameters
...
- **[`Asset Topology`][toop_engine_interfaces.asset_topology]**: Physical substation and switching device models
- **[`Stored Action Set`][toop_engine_interfaces.stored_action_set]**: Pre-computed topology actions and switching sequences
- **[`N-1 Definition`][toop_engine_interfaces.nminus1_definition]**: Contingency analysis specifications
- **[`Loadflow Results`][toop_engine_interfaces.loadflow_results]**: Standardized power flow output formats

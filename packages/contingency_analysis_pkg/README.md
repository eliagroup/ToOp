...
The Contingency Analysis package provides the implementation to the [`Load-flow-Service`][toop_engine_interfaces.messages.lf_service] interface. This enables a backend independent Contingency definition. Additional it offers either Polars or Pandas DataFrames as a return value.

## Usage:

- Run the N-1 analysis with the function [`get_ac_loadflow_results`][toop_engine_contingency_analysis.ac_loadflow_service.ac_loadflow_service.get_ac_loadflow_results]; depending on the network you provide, it will execute the contingency definition in either `pandapower` or `pypowsybl`.

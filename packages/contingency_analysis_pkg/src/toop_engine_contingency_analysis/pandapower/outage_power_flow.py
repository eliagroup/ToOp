...
from toop_engine_contingency_analysis.tracing import current_outage_span, set_attrs, step
...
def run_outage_power_flow(
    ...
    *basecase_net* (deep-copy of the network after the base-case load flow) is
    ...
    """
    with step("topology", **{"toop.n_outaged_elements": len(outaged_elements)}):
    ...
    try:
        if spps.conditions.empty:
            warm_runpp = _with_warm_start(net, method, merged_runpp)
            outage = current_outage_span()
            set_attrs(outage, **{"toop.lf.warm_start": warm_runpp is not merged_runpp})
            try:
                _run_power_flow(
                    net=net,
                    method=method,
                    runpp_kwargs=warm_runpp,
                )
                ...
                set_attrs(outage, **{"toop.lf.cold_retry": True})
                _run_power_flow(
                    net=net,
                    method=method,

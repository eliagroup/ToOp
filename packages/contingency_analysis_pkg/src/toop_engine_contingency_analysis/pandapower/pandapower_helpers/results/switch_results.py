...
def get_failed_switch_results(
    ...
    are filled with empty strings. The frame is flat polars (no index), matching the
    shape :func:`get_switch_results` produces on the converged path.
    ...
    switch_element_mapping : pl.DataFrame
        Mapping between switches and connected elements (``SwitchElementMappingSchema``
        columns). One failed row is emitted per distinct ``switch_id``.
    contingency : PandapowerContingency
    ...
    # the previous pandas drop_duplicates(), which kept first-occurrence order.
    switch_ids = switch_element_mapping.get_column("switch_id").unique(maintain_order=True)

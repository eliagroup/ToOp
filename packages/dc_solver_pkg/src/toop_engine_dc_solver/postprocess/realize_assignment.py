...
def heuristic_first(
    ...
    return jnp.where(
        ...
        current_switching_table.at[first_true, np.arange(requires_addition.shape[0])].set(True),
...
def heuristic_least_connected_busbar(
    ...
    current_switching_table = jnp.where(
        ...
        current_switching_table.at[first_hit, np.arange(requires_addition.shape[0])].set(True),
        current_switching_table,
    )
    ...
    current_switching_table = jnp.where(
        requires_addition[None, :],
        current_switching_table.at[selected_busbar_indices, np.arange(requires_addition.shape[0])].set(True),
        current_switching_table,
    )

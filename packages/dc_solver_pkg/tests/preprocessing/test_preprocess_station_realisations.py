...
def test_enumerate_station_realisations_limit_physical_reassignments(network_data_test_grid: NetworkData):
    ...
    assert network_data_1.branch_action_set_switching_distance is not None, "Branch action set switching distance should not be None"
    assert np.all(np.array(network_data_1.branch_action_set_switching_distance) <= 2), "All switching distances should be less than or equal to the global limit of 2"
    assert len(network_data_1.branch_action_set) == len(network_data_1.branch_action_set_switching_distance), "Branch action set length should match the length of switching distances"
    ...
    assert np.any(network_data.branch_action_set_switching_distance[0].max() > limit_for_first_sub), "At least one action for the first sub should have a switching distance greater than limit_for_first_sub"
    network_data_2 = enumerate_station_realisations(network_data, reassignment_limits=ReassignmentLimits(station_specific_limits={relevant_ids[0]: limit_for_first_sub}))
    assert np.all(np.array(network_data_2.branch_action_set_switching_distance) <= limit_for_first_sub), "The switching distances for the first sub should now be less than or equal to limit_for_first_sub"
    ...
    network_data_3 = enumerate_station_realisations(
        ...
        reassignment_limits=ReassignmentLimits(
            ...
            station_specific_limits={relevant_ids[0]: limit_for_first_sub}
        )
    )
    assert np.all(np.array(network_data_3.branch_action_set_switching_distance[0]) <= limit_for_first_sub), "The switching distances for the first sub should be less than or equal to limit_for_first_sub"
    assert np.any(np.array(network_data_3.branch_action_set_switching_distance[0]) > global_limit), "At least one action for the first sub should have a switching distance greater than the global limit"

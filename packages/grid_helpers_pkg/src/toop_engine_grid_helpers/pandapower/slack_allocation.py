"""
Utility functions for selecting and assigning slack generators, replacing sgens with gens, and analyzing generation-capable islands in pandapower networks.
"""
...
from pandapower.toolbox.grid_modification import _replace_group_member_element_type, \
    _adapt_result_tables_in_replace_functions, _adapt_profiles_in_replace_functions
...
def _slack_allocation_tie_break(df_min: pd.DataFrame) -> tuple[int, str]:
    """
        Returns the index of the selected (s)gen & the respective element type after applying tie break rules.
...
def replace_sgen_by_gen(
    ...
    """
        Replaces an sgen with a gen element. A new column "replaced_sgen" is created in net.gen which is
    ...
    # assign vm_pu set point
    if bus in net.res_bus.index:
        vm_pu = net.res_bus.at[bus, "vm_pu"]
    else:  # no result information to get vm_pu
        # use net.gen.vm_pu or net.ext_grid.vm_pu or set 1.0 pu
        if bus in net.gen.bus.values:
            vm_pu = net.gen.vm_pu.loc[net.gen.bus == bus].values[0]
        elif bus in net.ext_grid.bus.values:
            vm_pu = net.ext_grid.vm_pu.loc[net.ext_grid.bus == bus].values[0]
        else:
            vm_pu = 1.0
    ...
    # --- drop replaced sgens
    if retain_sgen_elm is True:
        net.sgen.loc[sgen, "in_service"] = False
        # populate replaced_by_gen sgen column
...
def get_buses_with_reference_sources(net: pp.pandapowerNet) -> set[int]:
    """
        Return all bus indices that have generators or static generators
        with a positive referencePriority value.
...
def assign_slack_gen_by_weight(net: pp.pandapowerNet, bus_idx_set: set[np.int64]) -> tuple[int, str]:
    """
        Selects the (s)gen index to be assigned as slack based on referencePriority rules.
...
def assign_slack_per_island(
    ...
    """
        Assigns one slack generator per valid island in the network after isolating specific elements.
    ...
    """
    if ((not net.sgen.empty and not "referencePriority" in net.sgen.columns)
            or (not net.gen.empty and not "referencePriority" in net.gen.columns)):

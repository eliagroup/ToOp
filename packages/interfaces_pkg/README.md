...
Canonical topology state is split between `TopologyMasterData` keyed by `bus_group_id` and runtime `MaterializedStation` snapshots; legacy `Topology` JSON is normalized onto that pair at load boundaries.

import logbook
import pytest
from toop_engine_interfaces.messages.preprocess_heartbeat_factory import (
    PreprocessStage,
    create_preprocess_heartbeat,
    create_preprocess_status_info,
...
def test_preprocess_status_info_creation():
    status = create_preprocess_status_info(
...
def test_preprocess_heartbeat_idle():
    heartbeat = create_preprocess_heartbeat(idle=True)
...
def test_preprocess_heartbeat_with_status_info():
    status = create_preprocess_status_info(preprocess_id="job456", runtime=5.0, stage="convert_to_jax_started", message="")
    heartbeat = create_preprocess_heartbeat(idle=False, status_info=status)
...
def test_preprocess_stage_literals(stage):
    ...
    status = create_preprocess_status_info(preprocess_id="job789", runtime=1.0, stage=stage, message="")

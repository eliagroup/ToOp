...
def test_get_paths_file_does_not_exist(pipeline_and_dc_cfg):
    import copy

    pipeline_params, _ = pipeline_and_dc_cfg
...
def test_prepare_importer_parameters(pipeline_and_dc_cfg):
    pipeline_params, _ = pipeline_and_dc_cfg
...
def test_run_pipeline(pipeline_and_dc_cfg, preprocessing_parameters):
    pipeline_cfg, dc_cfg = pipeline_and_dc_cfg
    ...
    topo_paths = run_pipeline(
        ...
        k_best_topos=10,  # Set to 10 to cover warning
    )
    # We test quality of the output in the benchmark
    assert len(topo_paths) > 0, "No topologies were generated by the pipeline."


def test_run_pipeline_no_optimization_stage(pipeline_and_dc_cfg, preprocessing_parameters):
    pipeline_cfg, dc_cfg = pipeline_and_dc_cfg
    pipeline_cfg = PipelineConfig(**pipeline_cfg)
    preprocessing_parameters = PreprocessParameters(**preprocessing_parameters)
    _, file_path, data_folder, _ = get_paths(pipeline_cfg)

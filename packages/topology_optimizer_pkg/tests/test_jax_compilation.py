...
def test_jax_no_tracing_flag() -> None:
    ...
    other_shapes_result = jnp.array([0, 1], dtype=jnp.int32)
    ...
    increment_jit(other_shapes_result)
    ...
    with jax.no_tracing():
        ...
        with pytest.raises(RuntimeError, match="re-tracing function increment"):
            increment_jit(other_dtypes_result)  # Non-traced array with other values should raise
        with pytest.raises(RuntimeError, match="re-tracing function increment"):
            increment_jit(other_values_result)  # Non-traced array with different dtype  should raise
        with pytest.raises(RuntimeError, match="re-tracing function increment"):
            increment_jit(other_shapes_result)


def _initialize_small_optimizer(static_information_file: str) -> tuple[DiscreteMapElites, JaxOptimizerData]:
...
def test_dc_optimization_does_not_retrace(
    ...
    """
    jax.clear_caches()
    algo, jax_data = _initialize_small_optimizer(static_information_file)
    ...
    with jax.no_tracing():
        ...
        with pytest.raises(RuntimeError, match="re-tracing function"):
            ...
            _, jax_data_new = _initialize_small_optimizer(static_information_file)

            run_single_device_epoch(jax_data_new, 1, algo.update)  # Should raise due to non-traced input.
